...
const QuizQuestion = () => {
    ...
    return (
        ...
        <div className='w-9/12 mx-24 mt-12 flex justify-between'>
            ...
            {
                quiz.map(data =>  <ShowQuestion key={data.id} data={data}></ShowQuestion>)
                 
            }
        </ul>
        <div className='w-3/12 '>
<h1>Total Correct Ans:</h1>
        </div>
        </div>
        <div className='bg-yellow-500 p-4 rounded absolute mb-0 mt-4 mx-2 flex'>
            <BackwardIcon className="h-6 w-6 mr-2 text-blue-500"/> <Link to='/topics'>Back To Another Quiz</Link> 
            <button > </button>
        </div>
